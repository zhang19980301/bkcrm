<template>
  <div id="app">
    <!-- 浏览器原生播放器 -->
    <audio :src="song.url" id="audio" controls style="display:none"></audio>
    <!-- 音乐播放器 -->
    <div id="music-container" class="audioplayer">
      <div class="ap-status">
        <!-- 播放列表 -->
        <div class="ap-playlist transmitList" v-if="listShow">
          <!-- 列表头部 -->
        ...
        <div class="ap-controller">
          <div class="ap-controller-left">
            <!-- 封面 -->
            <div class="ap-cover">
              <img :src="song.cover" style="width:100%;" alt="" />
            </div>
            <!-- 上一首 -->
...
export default {
  ...
  watch: {
    ...
    songLists() {
      this.getData()
      // this.songLists = Array.from(this.songLists);
    },
    // 当前播放的列表改变事件
    song: {
      handler: function() {
  ...
  methods: {
    // 获取数据，并放入当前播放列表
    getData() {
      console.log(this.song, 1);
      if (this.songLists.length === 0) {
        console.log("当前没有音源")
        return
      }
      this.song = this.songLists[0];
      // 初始化音频的监听事件
      this.audioInit();
      // 初始化进度条与音量的鼠标滑动事件
      this.slideProgress();
      ...
      audio.addEventListener(
        ...
        () => {
          switch (_this.playType) {
            case 1: // 列表循环
              _this.index =
                _this.index + 1 >= _this.songLists.length ? 0 : _this.index + 1;
              break;
            case 2: // 随机播放
              _this.index = Math.floor(Math.random() * _this.songLists.length);
              break;
            case 3: // 单曲循环
              break;
          }
          _this.song = _this.songLists[_this.index];
    ...
    skipFn(type) {
      switch (this.playType) {
        case 2: // 随机播放
          this.index = Math.floor(Math.random() * this.songLists.length);
          break;
        default:
          if (type == "skipBack") {
            ...
            this.index =
              this.index + 1 >= this.songLists.length
                ? this.songLists.length - 1
                : this.index + 1;
          }
          break;
      }

      this.song = this.songLists[this.index];
      this.playing = true;
      setTimeout(() => {
